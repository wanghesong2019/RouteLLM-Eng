...
用法：
    ...
    ROUTELLM_INFERENCE_BASE=http://<host>:6070 python scripts/test_inference_api.py
...
BASE = os.environ.get("ROUTELLM_INFERENCE_BASE", "http://127.0.0.1:6070")
...
def main() -> int:
    ...
    for _ in range(20):
        ...
        lat.append(ms)
    lat.sort()
    print(f"  min={lat[0]:.1f}ms  p50={lat[len(lat)//2]:.1f}ms  "
