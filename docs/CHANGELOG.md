...
| 批量（经转发链路） | 100 条 114.2ms（1.14ms/条） |
| 错误处理 | 服务不可达 → `RemoteInferenceError` ✓ |

**重要发现 — 影响 Docker 化部署位置**：

当推理服务所在主机防火墙**仅开放 SSH 端口**时，其服务端口从外部不可达
（需借助 SSH 隧道 + 反向代理暴露）。这直接决定容器与推理服务的部署位置取舍。
...
**新发现**：若私有域名被 DNS 解析到本机公网 IP，而本机无 443 监听
→ 不可达（NAT hairpin 行为）。下游 LLM 需改走可达的公网 API。
...
| # | 现象 | 根因 | 处置 |
|---|---|---|---|
| 1 | `apt-get install` 卡 6 分钟+ | Debian 官方源不可达 | 换国内镜像源 → **43 秒** |
...
| 6 | 部分环境构建极慢 | 网络差异，不同主机速度差别显著 | 在快的主机构建后传输镜像 |
